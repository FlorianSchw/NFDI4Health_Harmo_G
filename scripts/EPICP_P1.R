...
EPICP_P1_Var1 <- EPICP_P1_FFQ |> 
  filter(GROUP == 1) |> 
  group_by(ffq_id) |> 
  summarise(VEGETABLES_1 = sum(quant_grp17))

EPICP_P1_Var2 <- EPICP_P1_FFQ |> 
  filter(GROUP == 1 &
         subgroup1 == 1) |> 
  group_by(ffq_id) |> 
  summarise(VEGETABLES_2 = sum(quant_grp17))

EPICP_P1_Var3 <- EPICP_P1_FFQ |> 
  filter(GROUP == 2) |> 
  group_by(ffq_id) |> 
  summarise(VEGETABLES_3 = sum(quant_grp17))

EPICP_P1_Var4 <- EPICP_P1_FFQ |> 
  filter(GROUP == 2 &
         subgroup1 == 1) |> 
  group_by(ffq_id) |> 
  summarise(VEGETABLES_4 = sum(quant_grp17))


ffq_condensed_Epic <- Reduce(function(...) merge(..., by = "ffq_id", all.x = TRUE), list(EPICP_P1_Var1,
                                                                                         EPICP_P1_Var2,
                                                                                         EPICP_P1_Var3,
                                                                                         EPICP_P1_Var4))
  
EPICP_P1 <- dplyr::left_join(EPICP_P1, ffq_condensed_Epic, by = "ffq_id") |> 
  select(-ffq_id)
